...
export async function POST(req: Request) {
  try {
    ...
    await audit(supabase, user.id, "announcement_create", null, { kind, title });
...
export async function PATCH(req: Request) {
  try {
    ...
    await audit(supabase, user.id, "announcement_update", null, { id, ...updates });
...
export async function DELETE(req: Request) {
  try {
    ...
    await audit(supabase, user.id, "announcement_delete", null, { id });
    return NextResponse.json({ ok: true });
  } catch (err) {
    return errorResponse(err);
  }
